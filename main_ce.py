...
def cifar10_loader(arguments):
    ...
    # Datasets with split-specific transforms.
...
def train(train_loader, model, task_loss, topographic_loss, optimizer, epoch, arguments):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        # Forward pass: encoder returns embeddings and logits.
        embeddings, features = model(images)

        # Task loss on logits.
        task_loss_value = task_loss(features, labels)
        
        # Topographic term and parameters used for grad-norm matching.
        if arguments.topography_type == 'ws':
            # Local WS loss on the final linear classifier.
            ...
            # Global topographic loss on embeddings.
...
def main():
    ...
    # Adapter returning logits only (validate() expects logits).
    ...
    for epoch in range(1, arguments.epochs + 1):
        ...
        time1 = time.time()
        # Train end-to-end (topographic term handled inside train()).
    ...
    # Always save a final "last" snapshot.
    final_e2e = {
        'stage': 'e2e',
