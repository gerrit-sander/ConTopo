...
def setup_model(arguments):
    ...
    # Load CIFAR-10 metadata (class names + animacy mapping).
...
def train(train_loader, model, task_loss, topographic_loss, optimizer, epoch, arguments):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        # Topographic term depends on selected variant.
        if arguments.topography_type == 'ws':
            # Local WS loss on the final linear layer.
...
def main():
    ...
    for epoch in range(1, arguments.epochs + 1):
        ...
        # Combined validation loss for logging and checkpoint selection.
